...
<tr>
<td colspan="6">
<%if(p.isPrev()) {%><a href="userListPageMybatis.do?pageNum=<%=p.getStartPage()-1%>">[Prev]</a><%} %>
<%for(int i=p.getStartPage(); i<= p.getEndPage(); i++) {%>
	<%if(i == p.getPageNum()){%>
		<b>[<%=i %>]</b>
	<%}else{ %>
	<a href="userListPageMybatis.do?pageNum=<%=i%>">[<%=i %>]</a>
	<%} %>
<%} %>
<%if(p.isNext()){%><a href="userListPageMybatis.do?pageNum=<%=p.getEndPage()+1%>">[Next]</a><%} %>
</td>
</tr>
</table>
</h1>
